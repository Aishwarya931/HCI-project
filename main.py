...
@app.route('/noise_cancel', methods=['POST'])
def handle_noise_cancel():
    ...
    output_path = os.path.join(app.config['PROCESSED_FOLDER'], 'noise_cancelled_' + file.filename)
    noise_cancel(filepath, output_path)

    return send_from_directory(app.config['PROCESSED_FOLDER'], 'noise_cancelled_' + file.filename, as_attachment=True)
